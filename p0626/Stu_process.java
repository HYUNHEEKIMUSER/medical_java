...
public class Stu_process {
	...
	void stu_search(ArrayList list) {
		...
		String search_name = ""; // 이름 중복이여서 여기로 옮김
		int search_num = 0;
		double search_avg = 0;
		...
		case 1: //학생이름 검색
			...
			for(int i=0; i<list.size(); i++) {
				Students s = (Students)list.get(i);
				if(s.name.contains(search_name)) //contains 포함 되어있는 지{
					searchList.add(s);
			}
			break;
			
		case 2:
			System.out.println("몇 점 이상의 합계점수를 검색하겠습니까?>>");
			search_num = scan.nextInt();
			
			for(int i=0; i<list.size(); i++) {
				Students s = (Students)list.get(i);
				if(s.total>=search_num) //contains 포함 되어있는 지{
					searchList.add(s);
			}
			break;
		case 3:
			System.out.println("몇 점 이상의 평균점수를 검색하겠습니까?>>");
			search_avg = scan.nextDouble();
			
			for(int i=0; i<list.size(); i++) {
				Students s = (Students)list.get(i);
				if(s.avg>=search_avg) //contains 포함 되어있는 지{
					searchList.add(s);
			}
			break;
		}
