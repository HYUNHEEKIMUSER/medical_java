...
public class Stu_process {
	static Scanner scan = new Scanner(System.in); // 객체 선언 -> 클래스 변수가 되었음 ->클래스명.변수명 -> 같은 클래스내에서는 그냥 쓸 수 있음
	String stuNo;
	String name;
	int kor;
	int eng;
	...
	// 1. 화면 부분
	int screen() { //choice 타입으로 void->int로 바꿈
		//화면부분
	...
	// 1.학생성적입력부분
	int stu_input(Stu_score[] sc,int s_count) { //s_count는 int라서 void에서 int로 바꿈
		while(true) {
